...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme674
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme674_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme674_html_head_alter(&$head_elements) {
 ...
 */
function theme674_preprocess_search_block_form(&$vars) {
 ...
 */
function theme674_preprocess(&$vars, $hook) {
  $vars['theme674_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme674_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme674_doctype();
  $vars['rdf'] = _theme674_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme674_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme674_id_safe('section-' . $section);
 ...
 */
function theme674_preprocess_page(&$vars) {
 ...
 */
function theme674_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme674') . '/css/maintenance-page.css');
}
 ...
 */
function theme674_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme674_preprocess_block(&$vars, $hook) {
 ...
 */
function theme674_menu_tree($vars) {
 ...
 */
function theme674_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme674_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme674_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme674_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme674_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme674_breadcrumb_separator');
 ...
 * @return bool
 */
function theme674_tabs_float() {
  $float = (bool) theme_get_setting('theme674_tabs_float');
  $float_node = (bool) theme_get_setting('theme674_tabs_node');
...
function theme674_id_safe($string) {
  // Strip accents
  ...
  if (!ctype_lower($string)) { // Don't use ctype_alpha since its locale aware.
    $string = 'id' . $string;
  }
  return $string;
}
 ...
 * @ingroup themeable
 */
function theme674_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme674_id_safe($element['#title']);
 ...
 */
function theme674_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme674_menu_local_tasks(&$vars) {
  $output = '';
